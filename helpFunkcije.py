...
def minimizeReprojection(dof,d2dPoints1, d2dPoints2, d3dPoints1, d3dPoints2, w2cMatrix):
    ...
    errA = np.zeros((numPoints,3))
    errB = np.zeros((numPoints,3))
    ...
    residual = np.vstack((errA,errB))
    return residual.flatten()

def generisanje3DTacki(tacke2D_L, tacke2D_R, Proj1, Proj2):
    brojTacki = tacke2D_L.shape[0]
    d3dTacke = np.ones((brojTacki,3))
